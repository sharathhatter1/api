...
from models import WeatherCurrent, WeatherForecast, WeatherAlert
import allure
...
class WeatherAPI:
    ...
    @allure.step("Get current weather for {city}")
    def get_current_weather(self, city: str):
        response = self.client.get("/current.json", params={"q": city, "key": env.api_key})
        return response
